...
console.log("\n── §E  쇼룸 단독 진입(#showroom)이 종전대로 도는가 ──────────");
{
  ...
  //   ★ 26-09-09 → 26-09-10, **223 을 두 번 움직였다가 제자리로 돌아왔다.** 기록을 남긴다:
  //     ① 26-09-09 미구하라 견적서의 「350ab 국2」를 믿고 AB350 4×62 를 313,040 으로 내렸다
  //        → 표준사양(STD)이 AB350 이므로 이 수가 223 → **202** 로 떨어졌고, 그렇게 고쳤다.
  //     ② 26-09-10 실무자 확인: 그 견적서는 지종도 **CCP 350g** 이었다(대표가 잘못 적었다).
  //        313,040 은 CCP350 으로 옮겼고 AB350 은 378,612 로 원복 → 이 수도 **223 으로 복귀**.
  //     ⟹ 교훈은 숫자가 아니라 **출처**다. 「견적서에 적힌 지종·판형」은 틀릴 수 있고,
  //        판형은 up·전개도로 교차검증되지만 **지종은 기하로 확인할 방법이 없다** —
  //        새 지종 단가는 실무자 확인을 근거로 삼아라.
  //     ⚠ 이 줄이 또 빨개지면 먼저 paper-prices.mjs 의 **AB350 4x62** 를 봐라. 그때도
