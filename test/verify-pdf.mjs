...
// ★ 26-09-07 — 오라클은 곀사 폴더에 산다. 그 폴더가 움직이면 스위트가
//   「파일 없음」으로 발개진다 — 실제로 세 번 물렸다. 정확한 경로가 어긋나면
//   파일명으로 찾고, 찾았다고 **찍는다**(조용하면 낡은 경로가 안 고쳐진다).
...
const CASES = [
  {
    ...
    file: `${R}/고객사_진행중단/소스코/디자인/삼면접착140x43x130.pdf`,
    w: 380.3, h: 223.5,
  ...
  },
  {
    ...
    file: `${R}/고객사_진행중단/웨이크버니/디자인/웨이크버니삼면접착2종.pdf`,
  ...
  },
  {
    ...
    file: `${R}/고객사_진행중단/iSHAP/디자인/무제-3.pdf`,
  ...
  {
    name: "iSHAP 생활용품패키지",
    file: `${R}/고객사_진행중단/iSHAP/디자인/260805_코리팩_크리아_시안_생활용품패키지.pdf`,
    w: 557.1, h: 324.2,
  ...
  },
  {
    ...
    file: `${R}/고객사/제작완료/아르토_4차/디자인/260506_코리팩_크리아_디자인(13948)_클립클로우패키지.pdf`,
...
for (const c of CASES) {
  const label = c.name;
  // 낡은 경로를 살리는 것이 아니다 — 한 번 찾아 알리고 사람이 소스를 고치게 한다.
  c.file = resolveOracle(c.file, [R]) ?? c.file;
