...
// ★ 26-08-16 픽스처 교체 — **규칙은 무손상, 픽스처의 여유가 사라졌다**
//   맞뚜껑 폴리곤을 역텍 → 스트레이트텍으로 교정하니(tuck-both.mjs 참조) 도형이 커져
//   40×20×120 의 up 이 4x62 16→15 · 4x64 8→6 으로 내려갔고, 페널티 3% 가 더 이상
//   선택을 뒤집지 못해 두 줄이 **0/2** 가 됐다.
//   ⚠ 착각하지 마라 — 이건 「페널티가 깨졌다」가 아니다. 종전 두 케이스는 교정 후
//     페널티를 **꺼도 같은 판형**이 나온다(둘 다 ha2/4x63 추정 고정) = 게이트에 이빨이
//     없어진 상태다. 그대로 두면 페널티를 지워도 초록으로 통과한다.
//   대체 케이스는 「페널티 O → 실측 / 페널티 X → 추정」이 실제로 갈리는 것만 골랐다
//   (실코드 스캔 2,462건 중 구조·판형 조합이 겹치지 않는 2건).
//   ※ 이 표는 폴리곤에 묶여 있다. 날개 프로파일을 또 고치면 여기부터 확인해라 —
//     확인 방법은 EST_PRICE_PENALTY 를 0 으로 두고 같은 케이스를 돌려 판형이
//     **달라지는지** 보는 것이다. 안 달라지면 그 줄은 죽은 픽스처다.
//   ★ 26-08-19 두 번째 픽스처 교체 — 위 확인 절차를 그대로 밟았다.
//     맞뚜껑 텍 혀를 16.5 → 18.0 으로 올리니(tuck-both.mjs ② 「★★ 26-08-19」) netH 가
//     +3mm 되어 40×20×120 의 선택이 4x62(실측) → ha2(추정)로 넘어갔다. 그런데 그 줄은
//     **페널티를 꺼도 ha2** 다 — 즉 규칙이 깨진 게 아니라 픽스처가 또 죽었다(위 절차대로
//     EST_PRICE_PENALTY=0 으로 두고 확인함: ON/OFF 둘 다 ha2 · 528,523원).
//     실코드 스캔(3구조 × 5W × 4D × 4H × 7지종 × 2수량 = 1,680조합)에서 페널티가
//     선택을 뒤집는 케이스가 **174건** 남아 있고, 그중 원래 픽스처와 같은 모양
//     (4x62 실측 ↔ ha2 추정)인 것으로 갈아 끼웠다. 규칙은 무손상이다.
const EST = [
  // [이름, box, 지종, 수량, 기대 판형, 페널티 없을 때 나오던 판형]
  ["맞뚜껑 40×15×180 · AB400 20,000ea",
   { mode:"box", structure:"tuck_both", W:40, D:15, H:180, hangTab:0 }, "AB400", 20000,
   "4x62", "ha2(추정)"],
  ["맞뚜껑 40×15×120 · MK350 5,000ea",
   { mode:"box", structure:"tuck_both", W:40, D:15, H:120, hangTab:0 }, "MK350", 5000,
   "4x64", "4x63(추정)"],
];
