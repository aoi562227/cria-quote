...
console.log("케이스".padEnd(24)+"판형".padEnd(8)+"전개도".padEnd(16)+"견적서  실코드 판정   배치");
...
for(const [t,nm,W,D,H,sid,realUp] of Q){
  ...
  const how = `${layout.cols}×${layout.rows}${layout.rotated?" 회전":""}` +
              `${layout.interlocked?` ${layout.overlapInfo}`:" 칼선공유"}` +
              `${layout.utilCapped?" (발자국컷)":""}`;
  console.log(nm.padEnd(24)+sid.padEnd(8)+
    `${dieline.net.netW.toFixed(0)}×${dieline.net.netH.toFixed(0)}`.padEnd(16)+
    String(realUp).padStart(4)+String(up).padStart(7)+"   "+v.padEnd(8)+how);
}
console.log("─".repeat(96));
console.log(`일치 ${hit}/${Q.length}   부족 ${under}   초과 ${over}`);
console.log(`
※ 판은 **원지 규격**(BASE_SHEETS)에서 시작해 인쇄기 상한 990×720 으로 클램프하고
  물림 20/30 을 뺀 값이다. 견적서 up 은 **이미 재단된 「단위」** 위에서 나온 값이므로
  이 표는 물림을 이중으로 빼고 있을 수 있다. 판을 「단위」로 직접 주는 측정은
  scorecard-unit.mjs (§A 물림 0 / §C 물림 적용) 가 담당한다.`);
